...
document.getElementById("salesForm").addEventListener("submit", function (e) {
  ...
  if (tonnageSold <= 0)
    return showMessage("Tonnage must be greater than zero.");
  if (String(amountPaid).length < 5)
    return showMessage("Amount must be at least 5 digits.");
  if (buyerName.length < 2)
    return showMessage("Buyer name must be at least 2 characters.");
  if (agentName.length < 2)
    return showMessage("Sales agent name must be at least 2 characters.");
